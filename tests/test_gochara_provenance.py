...
def test_node_houses_are_verified_and_no_vedha_boundary_is_scoped():
    ...
    assert '3, 6, 10 and 11' in claim['scope']
    assert 'no-node-Vedha policy remains' in claim['scope']
...
def test_user_copy_does_not_launder_brihat_samhita_authority():
    ...
    assert 'Rahu/Ketu follow Phaladeepika 26.2' in html
