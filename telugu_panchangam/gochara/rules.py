...
def _occupants(janma_rasi: str, sky: dict[str, str]) -> dict[int, list[str]]:
    result: dict[int, list[str]] = {}
    for graha, rasi in sky.items():
        result.setdefault(_house_from(janma_rasi, rasi), []).append(graha)
    return result


def _vedha_by(graha: str, position: int, occupants: dict[int, list[str]]) -> str | None:
    if graha in _NODES:
        return None
    vedha_house = VEDHA[graha].get(position)
    for other in occupants.get(vedha_house, []):
        if (
            other != graha
            and other not in _NODES
            and (graha, other) not in _VEDHA_EXEMPT
        ):
            return other
    return None
...
def _gochara_entry(
    ...
    position = _house_from(janma_rasi, rasi)
    vedha_by = None
    verdict = 'adverse'
    if position in GOCHARA_FAVOURABLE[graha]:
        vedha_by = _vedha_by(graha, position, occupants)
        verdict = 'blocked' if vedha_by else 'favourable'
    return {
        'graha': graha,
        'rasi': rasi,
        'position': position,
...
def gochara_for(janma_rasi: str, sky: dict[str, str]) -> list[dict]:
    ...
    if janma_rasi not in RASHI_NAMES:
        raise ValueError(
            f'Unknown rashi {janma_rasi!r} — expected one of {RASHI_NAMES}'
        )
    occupants = _occupants(janma_rasi, sky)
    return [
        _gochara_entry(janma_rasi, graha, rasi, occupants)
        for graha, rasi in sky.items()
    ]
