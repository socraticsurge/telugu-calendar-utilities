...
def find_muhurta(
    ...
    """Find ranked auspicious time slots over the coming days. Slots are good choghadiya blocks (Amrit/Shubh/Labh/Char) with every inauspicious window subtracted (Rahu Kalam, Gulika, Yamagandam, Varjyam, Durmuhurtham), scored with Abhijit Muhurta / Amrita Kalam overlap and special-yoga bonuses. activity tunes the rules: travel additionally avoids Vishti karana, ceremony skips Visha/Dagdha days, purchase favours Labh, beginning favours Amrit. New in 1.9.0: Bhadra Mukha is a hard-avoid cut; Simha-Stha Guru/Shukra, Guru/Shukra Maudhya (combustion), Khar-Maasa, Adhika Maasa, and Pitru Paksha are all skipped for samskara activities; Panchaka Rahita (Mrityu/Agni/Raja/Chora/Roga) is checked at both day-level (sunrise lagna) and slot-level; optional travel_direction parameter activates Disha Shoola filtering. The legacy activity name litigation resolves to the verified court filing profile and has no separate Bhadra Puchha bonus. Pass janma_nakshatras (1-4 birth stars) to keep only days whose tarabalam favours everyone. Optionally pass janma_rasis (aligned with janma_nakshatras, null entries allowed) to add Chandrabalam scoring — each person then gets the Moon's position from their rashi with a verdict (good / needs remedial puja / avoid), and chandra_mode selects how this affects the day filter: 'stars' (annotate only, default), 'puja_ok' (drop Moon-avoid days), 'strict' (Moon must be good). Optionally pass janma_lagnas (aligned, null entries allowed) to use strict Lagna Shuddhi for that person — kendra/trikona/Ashtama count from the natal ascendant; otherwise we fall back to counting from janma_rasis (Chandra-Rashi-as-lagna tradition). include_night=True adds night choghadiya slots (sunset to next sunrise) scored by the same panchangam factors but without Rahu Kalam / Gulika / Yamagandam / Abhijit, and with Brahma Muhurta (+2) and Nishita Kala (+2) as night bonuses. Args: start_date=YYYY-MM-DD, days=1-14, activity=any|travel|purchase|ceremony|beginning|court|litigation|cremation|construction_roof|wood_cutting|well_digging|coronation, city=city name (or latitude+longitude), system=drik|surya_siddhanta|vakya, janma_rasis=optional birth rashis (e.g. ['Meena', 'Simha']), janma_lagnas=optional birth lagnas (e.g. ['Vrishabha', None]), chandra_mode=stars|puja_ok|strict, ayanamsa=lahiri|raman|krishnamurti|true_chitrapaksha (default: lahiri), travel_direction=optional compass direction (N/S/E/W/NE/NW/SE/SW) to activate Disha Shoola filtering for travel activity, include_night=false|true."""
