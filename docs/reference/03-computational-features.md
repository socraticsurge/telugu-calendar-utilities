...
| `personal.muhurta-slot-ranking` | Diagnose hard day gates; subtract hard windows; evaluate named day/night windows; score textual and personal factors; cap unresolved dosha; assign absolute/relative tier; on the Drik website only, post-screen deterministic candidate-time chart predicates | Days + profile + optional births/direction/duration; DashaFlow Lahiri planet Rashis + validated local Drik/Lahiri Lagna frame → diagnoses, ranked slots, locally projected Whole Sign houses, chart outcomes, reasons and manual checks | Python, MCP, generated data, website | Ranking weights and relative tiers are project heuristics; the chart screen covers [29 named predicates](54-muhurtam-election-chart-screening.md) across 14 activities, including a scoped partial/provisional Aksharabhyasa assessor; unresolved boundaries are review-gated rather than treated as a pass |
...
```mermaid
flowchart TB
    A["day + activity + optional birth profiles"] --> B{"hard day gate?"}
