...
| `personal.court-mesha-d1-d9-foundation` | Pass authoritative Mesha D1 immediately; otherwise derive guarded Lagna D9 only when sidecar and local D1 Rasis agree; aggregate sampled states with reject-first precedence | Canonical local Drik/Lahiri D1, exact agreeing sidecar Lagna degree, authority and transition coverage → pass/fail/unknown evidence and an unwired conditional-admission state | Python owner + TypeScript mirror | Raman supplies the Mesha D1-or-Navamsa clause; local-D1 authority, 0.01° D9 boundary guard and reject effect are explicit project policy; `specified_unwired` until issues 285 and 400 |
| `personal.court-sixth-house-natural-malefic-foundation` | Consume the shared graha-nature classifier; reject a resolved natural-malefic occupant of Whole Sign house 6; aggregate represented states with reject-first precedence | Complete canonical chart + validated local-Lagna frame + graha/phase/association transition coverage → pass/fail/unknown evidence and an unwired Court disposition | Python owner + TypeScript mirror | Raman supplies the no-malefic-in-sixth clause; classification, guarded unknowns, Whole Sign frame and reject effect remain separately disclosed conventions; `specified_unwired` until issue 400 |
| `personal.court-lagna-sixth-lord-separation-foundation` | Derive Lagna and sixth-house lords from the classical seven-graha table; measure undirected Whole Sign distance; pass only at exact opposition and aggregate every represented state | Canonical local Lagna + complete graha Rasis + Lagna and both lord-ingress coverage → derived lords, distance and pass/fail/unknown evidence | Python owner + TypeScript mirror + generated convention | Raman supplies only “as far apart as possible”; distance 6 is disclosed project policy, never a graduated score; nodes and pool-relative ranking are excluded; `specified_unwired` until issue 400 |
| `personal.court-peace-pattern-foundation` | Evaluate natural-benefic Kendra occupation OR a natural benefic in an odd Rasi receiving a full aspect from another natural benefic; require complete represented-window coverage | Complete canonical chart + local-Lagna, graha-Rasi, phase, association and aspect transitions → neutral pass/fail/unknown information | Python owner + TypeScript mirror + generated convention | Subject continuity and receiver direction are disclosed project interpretation; `inform` only, with no adverse legal inference or score/rank/tier/admission effect; `specified_unwired` until issue 400 |
| `personal.benefic-rasi-foundation` | Derive the target Rasi's classical lord and reuse its resolved natural nature | Target Rasi + complete canonical chart → pass/fail/unknown with lord evidence | Python owner + TypeScript mirror + generated convention | Occupancy, dignity, own-sign status, nodes, functional nature and event effects stay separate; `specified_unwired` after issue 408 |
| `personal.natural-graha-nature` | Validate all nine canonical grahas; classify fixed natures, guarded Chandra phase and same-Rasi Budha association; resolve reusable house-set witnesses with decisive three-valued precedence | Complete Drik/Lahiri Whole Sign chart + target houses → per-graha benefic/malefic/unknown natures and pass/fail/unknown witnesses | Python owner + TypeScript mirror + generated convention | Phaladeepika II.27 and BPHS 3.11 support the categories; the 0.02° guard, half-up quantization and same-Rasi association are explicit project conventions; no event effect is wired in issue 354 |
...
| `interpretation.same-rasi-conjunction` | Validate a complete nine-graha Rasi chart; compare Chandra independently with Kuja and Shani; aggregate samples with fail over unknown over pass | Exact chart or sampled half-open window → pass, fail or unknown with evidence | Python library; TypeScript parity mirror remains unwired | Raman supplies the event statement; same-Rasi computation and future Borrowing reject effect are separately registered project policies |
