...
| `personal.muhurta-slot-ranking` | Diagnose hard day gates; subtract hard windows; evaluate named day/night windows; score textual and personal factors; cap unresolved dosha; assign absolute/relative tier | Days + profile + optional births/direction/duration → diagnoses, ranked slots, scores, reasons and manual checks | Python, MCP, website | Ranking weights and relative tiers are project heuristics; “best available” is not absolute scriptural quality |
